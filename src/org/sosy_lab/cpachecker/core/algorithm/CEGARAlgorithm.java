...
public class CEGARAlgorithm implements Algorithm, StatisticsProvider {
  ...
  public static class CEGARStatistics implements Statistics {

    private final Timer totalTimer = new Timer();
    private final Timer refinementTimer = new Timer();
    private final Timer analysisTimer = new Timer();

    @SuppressFBWarnings(value = "VO_VOLATILE_INCREMENT",
        justification = "only one thread writes, others read")
    public static volatile int countRefinements = 0;
    ...
    private volatile int countAnalysis = 0;
    ...
    public void printStatistics(PrintStream out, Result pResult,
      ...
      if (countRefinements > 0) {
        ...
        out.println("CPU time for CEGAR algorithm:     " + TimeSpan.ofNanos(cputimeCEGAR).formatAs(TimeUnit.SECONDS));
        out.println("Time for refinements:             " + refinementTimer);
        out.println("CPU time for refinements:         " + TimeSpan.ofNanos(cputimeRef).formatAs(TimeUnit.SECONDS));
        ...
        out.println("Refinement times:             " + 0);
      }
      //assert(countAnalysis == countRefinements + 1):"Incorrect countRefinements";
      if(!(countAnalysis == countRefinements + 1 || countAnalysis == countRefinements)) {
        out.println("Number of analysis:                " + countAnalysis);
        throw new IllegalStateException();
      }
      if (countAnalysis > 0) {
        //out.println("First time for refinement:         " + firstAnalysisTime.formatAs(TimeUnit.SECONDS));
        out.println("First time for refinement:         " + analysisTimer.getAvgTime().formatAs(TimeUnit.SECONDS));
        out.println("Last time for refinement:          " + analysisTimer.getLengthOfLastInterval().formatAs(TimeUnit.SECONDS));
        out.println("Average time for analysis:      " + analysisTimer.getAvgTime().formatAs(TimeUnit.SECONDS));
        out.println("Number of analysis:                " + countAnalysis);
      }
    }
  }
  ...
  protected static TimeSpan firstRefinementTime;
  protected static TimeSpan firstAnalysisTime;
  private static long cputimeCEGAR = 0;
  private static long cputimeCEGARInterval = -1;
  private static long cputimeRef = 0;
  private static long cputimeRefInterval = -1;
  ...
  public AlgorithmStatus run(ReachedSet reached) throws CPAException, InterruptedException {
    ...
    long startCEGAR = -1;
    try {
      startCEGAR = ProcessCpuTime.read();
    ...
    try {
      ...
      do {
        ...
        if (refinementNecessary(reached)) {
          ...
          if(stats.analysisTimer.getNumberOfIntervals() == 1) {
            firstAnalysisTime = stats.analysisTimer.getSumTime();
          }
          if(CPAMain.isReuseSummary) {
            RefineHelper.getInstance().buildCurPrecisionFileCacheMap();
          }
          //System.out.println("refineing.......");
          try {
            refinementSuccessful = refine(reached);
          } catch (AssertionError smte) {
            //do nothing
          }

          //add by cailiming, after refine, do some clear work...
          if(CPAMain.isReuseSummary) {
            RefineHelper.getInstance().clearAll();
          }
        ...
        if(stats.countAnalysis > stats.countRefinements) {
          assert(stats.countAnalysis == stats.countRefinements + 1):"Incorrect countRefinements";
          stats.analysisTimer.stop();
          if(stats.analysisTimer.getNumberOfIntervals() == 1) {
            firstAnalysisTime = stats.analysisTimer.getSumTime();
          }
      ...
      long endCEGAR = -1;
      try {
        endCEGAR = ProcessCpuTime.read();
      } catch (JMException pE) {
        pE.printStackTrace();
      }
      cputimeCEGARInterval = endCEGAR - startCEGAR;
      cputimeCEGAR += cputimeCEGARInterval;
  ...
  private boolean refine(ReachedSet reached) throws CPAException, InterruptedException {
    ...
    try {
      ...
      long endRef = -1;
      try {
        endRef = ProcessCpuTime.read();
      } catch (JMException pE) {
        pE.printStackTrace();
      }
      cputimeRefInterval = endRef - startRef;
      cputimeRef += cputimeRefInterval;
      if(stats.refinementTimer.getNumberOfIntervals() == 1) {
        firstRefinementTime = stats.refinementTimer.getSumTime();
      }
    }
