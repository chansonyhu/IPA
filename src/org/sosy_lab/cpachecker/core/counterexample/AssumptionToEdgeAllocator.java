...
public class AssumptionToEdgeAllocator {
  ...
  private List<AExpressionStatement> createAssignmentsAtEdge(CFAEdge pCFAEdge, ConcreteState pConcreteState) {
    ...
    if (pCFAEdge.getEdgeType() == CFAEdgeType.DeclarationEdge) {
      ...
      List<AExpressionStatement> tmp = new ArrayList<>();
      for (CFAEdge singleEdge : (MultiEdge) pCFAEdge) {
        if(singleEdge.getEdgeType() == CFAEdgeType.DeclarationEdge) {
          tmp.addAll(handleDeclaration(((ADeclarationEdge) singleEdge).getDeclaration(),
                  pCFAEdge.getPredecessor().getFunctionName(),
                  pConcreteState));
        } else if (singleEdge.getEdgeType() == CFAEdgeType.StatementEdge) {
          tmp.addAll(handleStatement(pCFAEdge, ((AStatementEdge) singleEdge).getStatement(), pConcreteState));
        } else if (singleEdge.getEdgeType() == CFAEdgeType.FunctionCallEdge) {
          tmp.addAll(handleFunctionCall(((FunctionCallEdge) singleEdge), pConcreteState));
        }
      }
      return tmp;
