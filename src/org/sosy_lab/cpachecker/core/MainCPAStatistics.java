...
public class MainCPAStatistics implements Statistics, AlgorithmIterationListener {
  ...
  final Timer impacted_traverse_new_Time = new Timer();
  final Timer cpaCreationTime = new Timer();
  final Timer cfadiffTime = new Timer();
  private final Timer analysisTime = new Timer();

  public static long programCpuTime;
  public static long  analysisCpuTime = 0;
...
//        if(s == null )
//          continue;
//        if(s.getName().equals("Assumption Collection algorithm"))
//          continue;
  ...
  private void printTimeStatistics(PrintStream out, Result result, ReachedSet reached) {
    ...
    out.println("  Time for cfa diff:               " + cfadiffTime);
    out.println("  Time for loading CPAs:               " + cpaCreationTime);
    out.println("  Time for impacted traverse:          " + impacted_traverse_new_Time);
    ...
    if(!ImpactedHelper.getInstance().invariantsFile.toString().contains("null.txt")) {
      out.println("Time for normal Analysis:                     " + analysisTime);
    }
    out.println("CPU time for analysis:                 " + TimeSpan.ofNanos(analysisCpuTime+CPAchecker.cputimeInit).formatAs(TimeUnit.SECONDS));
    if(!ImpactedHelper.getInstance().invariantsFile.toString().contains("null.txt")){
      out.println("CPU time for impacted:                 " + TimeSpan.ofNanos(CPAchecker.impactedCpuTime).formatAs(TimeUnit.SECONDS));
      out.println("CPU time for normal analysis:          " + TimeSpan.ofNanos(CPAchecker.normalanalysisCpuTime).formatAs(TimeUnit.SECONDS));
      out.println("CPU time for init:                     " + TimeSpan.ofNanos(CPAchecker.cputimeInit).formatAs(TimeUnit.SECONDS));
    }
