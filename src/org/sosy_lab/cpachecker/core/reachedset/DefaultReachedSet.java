...
class DefaultReachedSet implements ReachedSet {
  ...
  public void updatePrecision(AbstractState s, Precision newPrecision) {
    ...
    if (oldPrecision == null) {
      ...
      // reached.remove(s);
      System.out.println("State needs to be in the reached set in order to change the precision.");
//      throw new IllegalArgumentException("State needs to be in the reached set in order to change"
//          + " the precision.");
    }
  }
  ...
  public Precision getPrecision(AbstractState state) {
    ...
    if(!CPAMain.noLazyExpansion && prec == null)
      return null;
    if((CPAMain.isReuseInvariant || ImpactedHelper.getInstance().isRestart) && prec == null)
      return null;
