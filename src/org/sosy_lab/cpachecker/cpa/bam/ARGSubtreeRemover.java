...
//      }
      ...
      if (removeSubtree(reachedSet, removeElement, newReducedRemovePrecision, pPrecisionTypes)) {
        ...
        Precision newPrecisionKey = null;
        if(newReducedRemovePrecision != null && newReducedRemovePrecision.size() >= 1)
          newPrecisionKey = newReducedRemovePrecision.get(0);
        bamCache.updatePrecisionForEntry(reducedRootState, reducedRootPrecision, rootSubtree, newPrecisionKey);
      }
    ...
    while (!remainingPathElements.isEmpty()) {
      ...
      if (starting) {
        if (callNodes.contains(currentElement)) {
          ...
          boolean flag = removeUnpreciseCacheEntriesOnPath(currentElement, reducedState,
              pNewPrecisions,
              currentBlock,
