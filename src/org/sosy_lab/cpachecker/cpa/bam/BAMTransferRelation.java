...
public class BAMTransferRelation implements TransferRelation {
  ...
  private Collection<? extends AbstractState> getAbstractSuccessorsWithoutWrapping(
    ...
    if(currentBlock != null && currentBlock.isReturnNode(node) && RefineHelper.rerunning) {
      System.out.println("Return: " + ((ARGState) pState).getStateId());
      return Collections.emptySet();
    }
  ...
  private Collection<AbstractState> analyseRecursiveBlockAndExpand(
    ...
    final Pair<ReachedSet, Collection<AbstractState>> pair =
            ...
            argCache.get(pCoveringLevel.getFirst(), pCoveringLevel.getSecond(), pCoveringLevel.getThird(), this);
    ...
    //code by CLM, put file cached fun call to curprecisionfilecachemap
    if(CPAMain.isReuseSummary && RefineHelper.isReusedFileCache(reached) && IncrementalCache.getInstance().isCanUseFileCache()) {
      Triple<AbstractState, Precision, Block> curTriple = Triple.of(initialState, pPrecision, pOuterSubtree);
      RefineHelper.getInstance().getCurPrecisionFileCacheList().add(curTriple);
    }
  ...
  public Collection<AbstractState> analyseBlockAndExpand(
    ...
    final Collection<Pair<AbstractState, Precision>> reducedResult =
            getReducedResult(entryState, precision, outerSubtree, reducedInitialState, reducedInitialPrecision);

    logger.log(Level.ALL, "Resulting states:", reducedResult);

    if(RefineHelper.breakFromSuccessorAnalysis && reducedResult.size() == 0) {
      System.out.println("No reducedResult when rerun fun");
    }
  ...
  private Collection<Pair<AbstractState, Precision>> getReducedResult(
          final AbstractState initialState, final Precision initialPrecision, final Block outerSubtree,
          final AbstractState reducedInitialState, final Precision reducedInitialPrecision)
    ...
    final Pair<ReachedSet, Collection<AbstractState>> pair =
            argCache.get(reducedInitialState, reducedInitialPrecision, currentBlock, this);
    ...
    //code by CLM, put file cached fun call to curprecisionfilecachemap
    if(CPAMain.isReuseSummary && RefineHelper.isReusedFileCache(reached) && IncrementalCache.getInstance().isCanUseFileCache()) {
      Triple<AbstractState, Precision, Block> curTriple = Triple.of(initialState, initialPrecision, outerSubtree);
      RefineHelper.getInstance().getCurPrecisionFileCacheList().add(curTriple);
    }
    ...
    argCache.put(reducedInitialState, reached.getPrecision(reached.getFirstState()), currentBlock, reducedResult, rootOfBlock, true);
  ...
  public AbstractState getRebuildState(final AbstractState rootState, final AbstractState entryState, final AbstractState expandedState) {
  ...
  private Collection<AbstractState> performCompositeAnalysisWithCPAAlgorithm(
    ...
    } else if (reached.hasWaitingState() && !RefineHelper.breakFromSuccessorAnalysis) {
  ...
  public ReachedSet createInitialReachedSet(AbstractState initialState, Precision initialPredicatePrecision) {
  ...
  ARGState computeCounterexampleSubgraph(ARGState target, ARGReachedSet reachedSet,
    ...
    final BAMCEXSubgraphComputer cexSubgraphComputer = new BAMCEXSubgraphComputer(
            ...
            abstractStateToReachedSet, expandedToReducedCache, logger, this);
    bamcexSubgraphComputer = cexSubgraphComputer;
  ...
  //add by CLM
  public Map<AbstractState, ReachedSet> getAbstractStateToReachedSet() {
    return abstractStateToReachedSet;
  }

  protected BAMCEXSubgraphComputer bamcexSubgraphComputer;

  public BackwardARGState computeCounterexampleSubgraphForBlock(ARGState currentState, ARGState reducedState, BackwardARGState childState) {
    return bamcexSubgraphComputer.computeCounterexampleSubgraphForBlock(currentState, reducedState, childState);
  }
  ...
  public AbstractState getReducedStateByExpanedState(AbstractState state) {
    return expandedToReducedCache.get(state);
  }

  public void removeExpanedStateToReducedState(AbstractState state) {
