...
class AutomatonTransferRelation extends SingleEdgeTransferRelation {
  ...
  private Collection<AutomatonState> getFollowStates(AutomatonState state, List<AbstractState> otherElements, CFAEdge edge, boolean failOnUnknownMatch) throws CPATransferException {
    ...
    boolean flag =
        IncrementalPAUtil.getInstance().waitSet.size()<=0 && IncrementalPAUtil.getInstance().numOfBot<=0;
    boolean flag1 = IncrementalPAUtil.getInstance().waitSet.size()>0 &&
        IncrementalPAUtil.getInstance().numOfBot>0 && !IncrementalPAUtil.isInit;
//    if (state == cpa.getBottomState() && IncrementalPAUtil.getInstance().waitSet.size()<=0 && IncrementalPAUtil.getInstance().numOfBot<=0) {
