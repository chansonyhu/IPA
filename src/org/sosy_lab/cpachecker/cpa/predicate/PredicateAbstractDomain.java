...
public class PredicateAbstractDomain implements AbstractDomain {
  ...
  ++stats.numCoverageChecks;
    ...
    if (e1.isAbstractionState() && e2.isAbstractionState()) {
      IncrementalPAUtil.getInstance().coverage_check += 1;
      ...
      if(result == true) {
        IncrementalPAUtil.getInstance().absStop += 1;
      }
      return result;
      ...
      IncrementalPAUtil.getInstance().coverage_check += 1;
      IncrementalPAUtil.getInstance().curNonAbsStop = false;
      if(e1.getMergedInto() == e2 && IncrementalPAUtil.getInstance().curTarget == 100) {
        // IncrementalPAUtil.getInstance().nonAbsStop += 1;
        IncrementalPAUtil.getInstance().curNonAbsStop = true;
      }
