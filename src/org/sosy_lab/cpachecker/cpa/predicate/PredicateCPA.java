...
public class PredicateCPA implements ConfigurableProgramAnalysis, StatisticsProvider, ProofChecker, AutoCloseable {
  ...
  public PredicatePrecision initialPrecision;
  ...
  protected PredicateCPA(Configuration config, LogManager logger,
    ...
    GlobalInfo.getInstance().predicateManager = predicateManager;
    transfer = new PredicateTransferRelation(this, blk, config, direction);
    GlobalInfo.getInstance().pred_trans = transfer;
    ...
    GlobalInfo.getInstance().storeRegionManager(regionManager);
    GlobalInfo.getInstance().storePathFormulaManager(pathFormulaManager);
  ...
  public PredicatePrecisionBootstrapper getPrecisionBootstraper() {
    return precisionBootstraper;
  }
}
