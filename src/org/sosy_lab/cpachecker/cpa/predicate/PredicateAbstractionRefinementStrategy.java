...
public class PredicateAbstractionRefinementStrategy extends RefinementStrategy {
  ...
  public PredicateSharing predicateSharing = PredicateSharing.LOCATION;
  public static enum PredicateSharing {
  ...
  private boolean sharePredicates = true;
  ...
  public PredicateAbstractionRefinementStrategy(final Configuration config,
    ...
    GlobalInfo.getInstance().predicateSharing = predicateSharing;
  }
  ...
  protected boolean performRefinementForState(BooleanFormula pInterpolant, ARGState interpolationPoint) {
    ...
    /*int locInstance;
    if(predicateState.getAbstractionLocationsOnPath().size() == 0) {
      locInstance = loc.getNodeNumber();
    } else {
      locInstance = predicateState.getAbstractionLocationsOnPath().get(loc);
    }*/
  ...
  protected void finishRefinementOfPath(ARGState pUnreachableState,
    ...
    UnmodifiableReachedSet reached = pReached.asReachedSet();
    //if(!reached.getPrecision(reached.getFirstState()).equals(reached.getPrecision(reached.getLastState()))) {
    //  throw new AssertionError();
    //}
    PredicatePrecision targetStatePrecision;
    if(reached.getLastState() == null)
      targetStatePrecision = extractPredicatePrecision(reached.getPrecision(reached.getFirstState()));
    else targetStatePrecision = extractPredicatePrecision(reached.getPrecision(reached.getLastState()));
    ...
    logger.log(Level.INFO, "Old predicate map is", basePrecision);
    ...
    if(ImpactedHelper.getInstance().isFunctionWide) {
      predicateSharing = PredicateSharing.FUNCTION;
    }
    ...
    logger.log(Level.INFO, "Predicate map now is", newPrecision);

    //assert newPrecision.getFunctionPredicates().equals(basePrecision.getFunctionPredicates()) : "We got same function predmap";
    ...
    if(basePrecision.getFunctionPredicates().equals(newPrecision.getFunctionPredicates())) {
      RefineHelper.samePrecision = true;
      RefineHelper.sptimes++;
      if((RefineHelper.sptimes > 10 && GlobalInfo.getInstance().spltimes > 10) && ImpactedHelper.getInstance().lastVFileDisjInvariants.size() > 0) {
        GlobalInfo.getInstance().lastInvSize = ImpactedHelper.getInstance().lastVFileDisjInvariants.size();
        ImpactedHelper.getInstance().lastVFileDisjInvariants.clear();
        //newPrecision = basePrecision.addFunctionPredicates(mergePredicatesPerFunction(newPredicates));
        //newPrecision = newPrecision.addFunctionPredicates(mergePredicatesPerFunction(newPrecision.getLocationInstancePredicates()));
        ...
        pReached.cutOffSubtree(refinementRoot);

        logger.log(Level.FINEST, "Restarting analysis after",refinementCount,"refinements by clearing the ARG.");
        System.out.println("Restarting analysis after" + refinementCount + "refinements by clearing the ARG.");
        CPAMain.isReuseInvariant = false;
        ImpactedHelper.getInstance().isRestart = true;
        newPrecision = ImpactedHelper.getInstance().funcWidePrecision;
        //newPrecision.mLocalPredicates.clear();
        RefineHelper.sptimes = 0;
        GlobalInfo.getInstance().spltimes = 0;
        //refinementCount = 0;

      }
      //if(!CPAMain.noLazyExpansion) {
      //  newPredicates = null;
      //  return;
      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////}
    }
