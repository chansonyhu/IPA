...
public class PredicateTransferRelation extends SingleEdgeTransferRelation {
  ...
  public Collection<? extends AbstractState> getAbstractSuccessorsForEdge(
    ...
    try {
      ...
      PrecisionHelpState phHelp = new PrecisionHelpState(loc.getFunctionName(), pPrecision, new PredicateAnalysisPrecisionExtractor());

      Pair<Integer, String> key = Pair.of(loc.getNodeNumber(), phHelp.toString());
      ...
      // if(false) {
        PathFormula pathFormula = ((PredicateAbstractState) pElement).getPathFormula();
        AbstractionFormula abstractionFormula =
            ((PredicateAbstractState) pElement).getAbstractionFormula();
      ...
      if(ImpactedHelper.getInstance().lastVFileInvariants.containsKey(key) && !ImpactedHelper.getInstance()
          .isInvDisj && IncrementalPAUtil.isInit) {
        PredicateAbstractState pS = (PredicateAbstractState) ImpactedHelper.getInstance().lastVFileInvariants.get(key);
        AbstractionFormula abstractionFormula = pS.getAbstractionFormula();
        boolean isHit = ImpactedHelper.getInstance().checkImpacted(abstractionFormula);
        if(isHit) {
            ImpactedHelper.isHit = true;
            ImpactedHelper.hits++;
            return Collections.singleton(ImpactedHelper.getInstance().lastVFileInvariants.get(key));
        }
      }
      ...
      if(doAbstraction) {
        ImpactedHelper.getInstance().doAbstractionNodeCount++;
        ImpactedHelper.getInstance().doAbstractionNodeNum.add(loc.getNodeNumber());
      }
