...
public class ARGState extends AbstractSingleWrapperState implements Comparable<ARGState>, Graphable, Serializable{
  ...
  public final Collection<ARGState> children = new ArrayList<>(1);
  public final Collection<ARGState> parents = new ArrayList<>(1);
  public int target = -1;
  ...
  public PrecisionAdjustmentResult.Action action;
...
//    assert !destroyed : "Don't use destroyed ARGState " + this;
  ...
  public void deleteChild(ARGState child) {
  ...
  public boolean isTarget() {
    //TODO qianshan may not be sound!
  ...
  public void removeChild(ARGState pChild) {
    ArrayList<ARGState> tmp = new ArrayList<>();
    tmp.addAll(children);
    for(ARGState child : tmp) {
      if(child.equals(pChild)) {
        children.remove(pChild);
      }
    }
    tmp.clear();
    tmp.addAll(pChild.parents);
    for(ARGState father : tmp) {
      if(father.equals(this))
          pChild.parents.remove(this);
    }
  }
  public void removeFather(ARGState pFather) {
    for(ARGState father : parents) {
      if(father.equals(pFather))
        parents.remove(pFather);
    }
    for(ARGState child : pFather.children) {
      if(child.equals(this))
        pFather.children.remove(this);
    }
  }
  public void childrenAddParentTo(ARGState replacement) {
    for (ARGState child : children) {
  ...
  public void childrenDelParentTo(ARGState target) {
    BAMCEXSubgraphComputer.BackwardARGState tmp = ((BAMCEXSubgraphComputer.BackwardARGState)(this.children.toArray()[0]));
    tmp.parents.remove(target);
    target.children.clear();
    //this.parents.remove(tmp);
  }
  public void replaceInARGWith(ARGState replacement, boolean isExpand) {
    // replacement here is original state in the error path
    if(!isExpand) {
      replaceInARGWith(replacement);
      return;
    }
    assert !destroyed : "Don't use destroyed ARGState " + this;
