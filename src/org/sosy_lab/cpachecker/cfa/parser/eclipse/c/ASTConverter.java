...
class ASTConverter {
  ...
  //add by CLM
  public static String curFunctionName;
...
//            e, niceFileNameFunction);
  ...
  private CRightHandSide convert(IASTFunctionCallExpression e) {
    IASTFunctionCallExpression fCallExpression = e;
    String curFName = fCallExpression.getFunctionNameExpression().getRawSignature();
    if(!ProgramInfo.getInstance().getNowFunctionInfo().containsKey(curFName)) {
    }
    else {
      ProgramInfo.getInstance().getNowFunctionInfo().get(curFunctionName).addCalledFunctionName(curFName);
    }
  ...
  public CFunctionDeclaration convert(final IASTFunctionDefinition f) {
    ...
    IASTFunctionDeclarator fDeclarator = f.getDeclarator();
    IASTNode[] nodes = fDeclarator.getChildren();
    String tmpFunctionName = null;//if function is pointer function
    for(int i = 0; i < nodes.length; i++) {
      if(nodes[i] instanceof CASTName) {
        curFunName = nodes[i].getRawSignature();
      }
      if(nodes[i] instanceof CASTDeclarator) {
        tmpFunctionName = nodes[i].getRawSignature();
      }
    }
    if(curFunName == null) {
      curFunName = tmpFunctionName;
    }
    ...
    if(ProgramInfo.getInstance().getNowFunctionInfo().containsKey(curFunName)) {
      ...
      curFunctionInfo.setFunctionBody(f.getBody().getRawSignature());

      if(curFunName.equals(CPAMain.entryFunction)) {
        ProgramInfo.getInstance().setMainInfo(curFunctionInfo);
      }
    }
    else {
      FunctionInfo curFunctionInfo = new FunctionInfo(curFunName, f.getBody().getRawSignature());
