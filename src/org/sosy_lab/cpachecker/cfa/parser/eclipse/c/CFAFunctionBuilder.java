...
class CFAFunctionBuilder extends ASTVisitor {
  ...
  private int handleSimpleDeclaration(final IASTSimpleDeclaration sd) {
    ...
    //qianshan
    //nextNode.impactedVariables.addAll(prevNode.impactedVariables);

    if(ImpactedHelper.getInstance().isCurImpacted) {
      String target;
      String decStr = sd.getRawSignature();
      Boolean match = false;
      int i = 0;
      CFAEdge edge = null;
      for(i = 0; i < prevNode.getNumLeavingEdges(); i++) {
        edge = prevNode.getLeavingEdge(i);
        target = edge.getRawStatement();
        if(target.equals(decStr)) {
          match = true;
          break;
        }
      }
      if(match) {
        nextNode.impactedVariables.add(((CDeclarationEdge) edge).getDeclaration());
        int curImpactedNum = nextNode.impactedVariables.size();
  ...
  public int visit(IASTStatement statement) {
    ...
    //TODO qianshan Impacted
    if(ImpactedHelper.getInstance().impactedLines.contains(fileloc.getStartingLineNumber())
    && !(statement instanceof IASTIfStatement) && !(statement instanceof IASTWhileStatement)) {
      ImpactedHelper.getInstance().isCurImpacted = true;
    }
    ...
    if (statement instanceof IASTCompoundStatement) {
      //TODO qianshan impacted: do not handle compoundstatement
      if(ImpactedHelper.getInstance().isCurImpacted) {
        ImpactedHelper.getInstance().isCurImpacted = false;
      }
      ...
      handleExpressionStatement((IASTExpressionStatement)statement, fileloc);
        ...
        IASTFileLocation conditionLoc = ((CASTIfStatement) statement).getConditionExpression().getFileLocation();
        assert conditionLoc.getStartingLineNumber() == conditionLoc.getEndingLineNumber();
        if(ImpactedHelper.getInstance().impactedLines.contains(conditionLoc.getStartingLineNumber())
                && ImpactedHelper.getInstance().externalDiff) {
            ImpactedHelper.getInstance().isCurImpacted = true;
        }
      handleIfStatement((IASTIfStatement)statement, fileloc);
    } else if (statement instanceof IASTWhileStatement) {
      IASTFileLocation conditionLoc = ((CASTWhileStatement) statement).getCondition().getFileLocation();
      assert conditionLoc.getStartingLineNumber() == conditionLoc.getEndingLineNumber();
      if(ImpactedHelper.getInstance().impactedLines.contains(conditionLoc.getStartingLineNumber())
              && ImpactedHelper.getInstance().externalDiff) {
        ImpactedHelper.getInstance().isCurImpacted = true;
      ...
      //TODO qianshan impacted
      logger.log(Level.INFO, ((IASTForStatement) statement).getBody().toString());
    ...
    checkArgument(!ImpactedHelper.getInstance().isCurImpacted);
    return PROCESS_CONTINUE;
  }
  ...
  private void handleExpressionStatement(IASTExpressionStatement exprStatement,
    ...
    if(ImpactedHelper.getInstance().isCurImpacted && ImpactedHelper.debug_canUseUnpreciseDiff) {
      CFAEdge edge = prevNode.getLeavingEdge(0);
      assert exprStatement.getExpression() instanceof CASTBinaryExpression;
      //CASTIdExpression astId = (CASTIdExpression) ((CASTBinaryExpression) exprStatement.getExpression()).getOperand1();
      //String astName = ((CASTIdExpression) ((CASTBinaryExpression) exprStatement.getExpression()).getOperand1()).getName().toString();
      assert exprStatement.getRawSignature().equals(edge.getRawStatement());
      ImpactedCFAUtils.decideIVonStatementEdge((CStatementEdge) edge, lastNode);
    }
  ...
  private void handleDeclarationStatement(IASTDeclarationStatement declarationStatement) {
    //CFANode prevNode = locStack.pop();
    CFANode lastNode = null;
    //lastNode.impactedVariables.addAll(prevNode.impactedVariables);
    //ImpactedHelper.getInstance().isCurImpacted = false;
  }
  ...
  private void handleIfStatement(IASTIfStatement ifStatement,
    ...
    if(ImpactedHelper.getInstance().isCurImpacted && ImpactedHelper.debug_canUseUnpreciseDiff) {
      if(thenNode.getNumEnteringEdges() > 0) {
        CFAEdge conditionEdge = thenNode.getEnteringEdge(0);
        if(conditionEdge instanceof BlankEdge) {
          ImpactedHelper.getInstance().isCurImpacted = false;
          return;
        }
        CExpression condition = ((CAssumeEdge) conditionEdge).getExpression();
        assert condition instanceof CBinaryExpression;
        CExpression op1 = ((CBinaryExpression) condition).getOperand1();
        CExpression op2 = ((CBinaryExpression) condition).getOperand2();
        if (op1 instanceof CIdExpression) {
          prevNode.impactedVariables.add(((CIdExpression) op1).getDeclaration());
        }
        if (op2 instanceof CIdExpression) {
          prevNode.impactedVariables.add(((CIdExpression) op2).getDeclaration());
        }
        int curImpactedNum = prevNode.impactedVariables.size();
        if (curImpactedNum > ImpactedHelper.getInstance().maxImpactedNum) {
          ImpactedHelper.getInstance().maxImpactedNum = curImpactedNum;
        }
      }
  ...
  private void handleWhileStatement(IASTWhileStatement whileStatement, FileLocation fileloc) {
    ...
    createLoop(whileStatement.getCondition(), whileStatement.getBody().getRawSignature(), fileloc);
  ...
  private void handleDoWhileStatement(IASTDoStatement doStatement, FileLocation fileloc) {
    ...
    createLoop(doStatement.getCondition(), doStatement.getBody().getRawSignature(), fileloc);
  ...
  private void saveLoopInformation(CFANode loopStart, String signature) {
    //TODO qianshan
    String curFunName = "loop";
    curFunName = curFunName + (loopStart.getNodeNumber() - 1);
    logger.log(Level.INFO, curFunName);
    if(ProgramInfo.getInstance().getNowFunctionInfo().containsKey(curFunName)) {
      FunctionInfo curFunctionInfo = ProgramInfo.getInstance().getNowFunctionInfo().get(curFunName);
      curFunctionInfo.setFunctionBody(signature);
    }
    else {
      FunctionInfo curFunctionInfo = new FunctionInfo(curFunName, signature);
      ProgramInfo.getInstance().getNowFunctionInfo().put(curFunName, curFunctionInfo);
    }
    if(!ProgramInfo.getInstance().getNowFunctionInfo().containsKey(curFunName)) {
    }
    else {
      ProgramInfo.getInstance().getNowFunctionInfo().get(ASTConverter.curFunctionName).addCalledFunctionName(curFunName);
    }
    ASTConverter.curFunctionName = curFunName;
  }
   ...
   * @category loops
   */
  private void createLoop(IASTExpression condition, String signature, FileLocation fileloc) {
    ...
    saveLoopInformation(loopStart, signature);

    if(ImpactedHelper.getInstance().isCurImpacted && ImpactedHelper.debug_canUseUnpreciseDiff) {
      CFAEdge conditionEdge = loopStart.getLeavingEdge(0);
      CExpression cond = ((CAssumeEdge)conditionEdge).getExpression();
      assert cond instanceof CBinaryExpression;
      CExpression op1 = ((CBinaryExpression) cond).getOperand1();
      CExpression op2 = ((CBinaryExpression) cond).getOperand2();
      if(op1 instanceof CIdExpression) {
        loopStart.impactedVariables.add(((CIdExpression) op1).getDeclaration());
      }
      if(op2 instanceof CIdExpression) {
        loopStart.impactedVariables.add(((CIdExpression) op2).getDeclaration());
      }
      int curImpactedNum = loopStart.impactedVariables.size();
      if (curImpactedNum > ImpactedHelper.getInstance().maxImpactedNum) {
        ImpactedHelper.getInstance().maxImpactedNum = curImpactedNum;
      }
      ImpactedHelper.getInstance().isCurImpacted = false;
  ...
  private int handleForStatement(final IASTForStatement forStatement,
    ...
    int loopNumber = loopInit == loopStart ? prevNode.getNodeNumber() : loopStart.getNodeNumber();
    loopName = loopName + loopNumber;
    logger.log(Level.INFO, loopName);
    if(ProgramInfo.getInstance().getNowFunctionInfo().containsKey(loopName)) {
      FunctionInfo curFunctionInfo = ProgramInfo.getInstance().getNowFunctionInfo().get(loopName);
      curFunctionInfo.setFunctionBody(forStatement.getBody().getRawSignature());
    }
    else {
      FunctionInfo curFunctionInfo = new FunctionInfo(loopName, forStatement.getBody().getRawSignature());
      ProgramInfo.getInstance().getNowFunctionInfo().put(loopName, curFunctionInfo);
    }

    if(!ProgramInfo.getInstance().getNowFunctionInfo().containsKey(loopName)) {
    }
    else {
      ProgramInfo.getInstance().getNowFunctionInfo().get(ASTConverter.curFunctionName).addCalledFunctionName(loopName);
    }

    String tmpName = ASTConverter.curFunctionName;
    ASTConverter.curFunctionName = loopName;
    // visit only loopbody, not children, loop.getBody() != loop.getChildren()
    forStatement.getBody().accept(this);
    ASTConverter.curFunctionName = tmpName;
