...
public class CFANode implements Comparable<CFANode>, Serializable {
  ...
  //TODO qianshan
  public static boolean isBelongtoLoopBlock(CFANode node){
    HashSet<CFANode> loopNodes = new HashSet<>();
    for(Block block : BAMCPA.getPartitioning().getBlocks()){
      if(!block.isFunctionBlock())
        loopNodes.add(block.getCallNode());
    }
    for(CFANode loopnode : loopNodes){
      if(loopnode.getNodeNumber() == node.getNodeNumber())
        return true;
    }
    return false;
  }

  public static UniqueIdGenerator idGenerator = new UniqueIdGenerator();

  private final int nodeNumber;

  private final List<CFAEdge> leavingEdges = new ArrayList<>(1);
  private final List<CFAEdge> enteringEdges = new ArrayList<>(1);
  public Set<CSimpleDeclaration> impactedVariables = new HashSet<>();
  public Set<CSimpleDeclaration> initialVariables = new HashSet<>();
  public boolean isInitialImpacted = false;
  ...
  public CFANode(String pFunctionName) {
    ...
    if(functionName.equals("DUMMY_NODE"))
      nodeNumber = -1;
    else
