...
public class FormulaManagerFactory {
  ...
  public FormulaManagerFactory(Configuration config, LogManager pLogger,
    ...
    ImpactedHelper.getInstance().setManagers();
    //ImpactedHelper.getInstance().formulaManager = fmgr;
  ...
  private FormulaManager instantiateSolver(Solvers solver, Configuration config)
    ...
    try {
      switch (solver) {
      case SMTINTERPOL:
        if(ImpactedHelper.getInstance().isQEliminate) {
          GlobalInfo.getInstance().z3fm = Z3FormulaManager.create(logger, config, shutdownNotifier, logfile, randomSeed);
        }
      ...
      String msg = "The SMT solver " + solver
          + " is not available on this machine because of missing libraries"
          + " (" + e.getMessage() + ")."
          + " You may experiment with SMTInterpol by setting cpa.predicate.solver=SMTInterpol.";
      if(ImpactedHelper.getInstance().isQEliminate) {
        msg = "The SMT solver z3 "
            + " is not available on this machine because of missing libraries"
            + " (" + e.getMessage() + ").";
      }
...
//        + ")\\..*");
  ...
  public static WeakReference<ClassLoader> smtInterpolClassLoader = new WeakReference<>(null);
      ...
      synchronized (this) {
        ...
        if (result == null) {
          try {
            ...
            GlobalInfo.classloader = classLoader;
  ...
  public static ClassLoader getClassLoader(LogManager logger) {
