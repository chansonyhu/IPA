...
public class PathFormulaManagerImpl implements PathFormulaManager {
  ...
  @Override
  public PathFormula makeOrLight(final PathFormula pathFormula1, final PathFormula pathFormula2) throws InterruptedException {

    final BooleanFormula formula1 = pathFormula1.getFormula();
    final BooleanFormula formula2 = pathFormula2.getFormula();

    final BooleanFormula newFormula = bfmgr.or(formula1, formula2);

    final int newLength = Math.max(pathFormula1.getLength(), pathFormula2.getLength());

    return new PathFormula(newFormula, SSAMap.emptySSAMap(), PointerTargetSet.emptyPointerTargetSet(), newLength);
  }
