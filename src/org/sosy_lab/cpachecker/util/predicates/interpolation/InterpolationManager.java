...
public final class InterpolationManager {
  ...
  public static boolean isNewEnv = false;
  ...
  public class Interpolator<T> {
    ...
    private boolean checkInfeasabilityOfTrace(
      ...
      FormulaManager interpolationFormulaManager = null;
      FormulaManagerFactory factory = null;
      try {
        factory = new FormulaManagerFactory(config, logger, shtdwn);
      ...
      try {
        interpolationFormulaManager = SmtInterpolFormulaManager.create(config, logger, shtdwn, null, 42);
      } catch (InvalidConfigurationException e) {
      }
      ...
      itpProver = (InterpolatingProverEnvironment<T>) ipeA;
      */
      //itpProver = (InterpolatingProverEnvironment)fmgr.newProverEnvironmentWithInterpolation(true);
      //fmgr.newProverEnvironmentWithInterpolation(true);
      ...
      while (todoIterator.hasNext()) {
        ...
        if(RefineHelper.lazyBFCache.contains(f) && RefineHelper.isExpanding) {
        ...
        if(!bfmgr.isTrue(f)) {
          RefineHelper.lazyBFCache.add(f);
        }
      }
