...
public class SmtInterpolUtil {
  ...
  public static Term replaceArgs(SmtInterpolEnvironment env, Term t, Term[] newParams) {
    if (t instanceof ApplicationTerm) {
      ...
      String funcName = funcSymb.getName();
      Term newTerm;
//      if((funcName.equals("and") || funcName.equals("or")) && newParams.length <= 2) {
//        newTerm = env.term(funcName, funcSymb.getIndices(), funcSymb.getReturnSort(), newParams);
//      } else {
//        newTerm = env.term(funcName, funcSymb.getIndices(), null, newParams);
//      }
      newTerm = env.term(funcName, funcSymb.getIndices(), null, newParams);
      return newTerm;
    } else {
