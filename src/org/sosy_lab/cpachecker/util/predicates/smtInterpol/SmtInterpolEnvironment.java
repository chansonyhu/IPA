...
public class SmtInterpolEnvironment {
  ...
  public SmtInterpolEnvironment(Configuration config, final ShutdownNotifier pShutdownNotifier,
...
//        });
    ...
    logger.setLoglevel(DefaultLogger.LOGLEVEL_WARN);

    //logger = CPAMain.logManager;
  ...
  private Script createLoggingWrapper(SMTInterpol smtInterpol) {
    ...
    try {
      ...
      logger.warn(e + "Coud not open log file for SMTInterpol queries");
...
//  }
  ...
  SmtInterpolInterpolatingProver getInterpolator(SmtInterpolFormulaManager mgr) {
    if (smtLogfile != null) {
      ...
      try {
        ...
        logger.warn(e + "Could not write interpolation query to file");
  ...
  public List<Term> parseStringToTerms(String s) {
    ...
    ParseEnvironment parseEnv = new ParseEnvironment(parseScript, options) {
      @Override
