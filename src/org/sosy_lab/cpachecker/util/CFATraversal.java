    ...
    while (!toProcess.isEmpty()) {
      ...
      if (result != TraversalProcess.SKIP) {
        for (CFAEdge edge : edgeSupplier.apply(n)) {
          ...
          if (result != TraversalProcess.SKIP) {
            CFANode cur = successorSupplier.apply(edge);
            toProcess.addLast(cur);
            if(n instanceof CFunctionEntryNode && !n.isInitialImpacted) {
                n.impactedVariables.addAll(ImpactedHelper.getInstance().inpactedGlobalInitializedVariables);
              int curImpactedNum = n.impactedVariables.size();
              if (curImpactedNum > ImpactedHelper.getInstance().maxImpactedNum) {
                ImpactedHelper.getInstance().maxImpactedNum = curImpactedNum;
              }
                n.isInitialImpacted = true;
            }
            if(!n.impactedVariables.isEmpty() && !cur.isInitialImpacted) {
              //TODO we need to handle Global variable here
              cur.impactedVariables.addAll(n.impactedVariables);
              int curImpactedNum = cur.impactedVariables.size();
              if (curImpactedNum > ImpactedHelper.getInstance().maxImpactedNum) {
                ImpactedHelper.getInstance().maxImpactedNum = curImpactedNum;
              }
              if(n instanceof CFunctionEntryNode && !n.getFunctionName().equals(CPAMain.entryFunction)) {

              }
              cur.isInitialImpacted = true;
            }
//            if(cur.getNodeNumber() == 3) {
//              CFAEdge in = cur.getEnteringEdge(0);
//              if(in instanceof CDeclarationEdge) {
//                CVariableDeclaration vd = (CVariableDeclaration) ((CDeclarationEdge) (in)).getDeclaration();
//                if(vd.getName().equals("add")) {
//                  cur.impactedVariables.add(vd);
//                  cur.isInitialImpacted = true;
//                }
//              }
//            }
            //TODO add impact analysis here (update impactedVariables of CFANode) qianshan
          }
        }
      }
    }
