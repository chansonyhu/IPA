...
public class CPAMain {
    ...
    public static boolean cancelBAM = true;

    public static boolean lazySucc = false;
    public static String summaryOutputPath;
    public static String summaryInputPath;
    public static String invariantInputPath;
    public static String invariantOutputPath;
    public static boolean isReuseSummary = false;
    public static boolean isReusePrecision = false;
    public static boolean isReuseInvariant = false;
    public static boolean onlyLoopSummary = false;
    public static boolean loopSummary = false;
    public static boolean onlyFunctionSummary = false;
    public static boolean isUesTextSummary = false;
    public static boolean noLazyExpansion = true;
    public static boolean useReduction = true;
    public static boolean isSubgraph = false;
    public static boolean isHeuristicAddingMultiEdge = false;
    public static AbstractType abstractType = AbstractType.VALUEANALYSIS;
    public static String entryFunction = "main";
    public static String program;
    public static boolean isDebug = false;
    public static boolean isIncPred = false;

    public static CPAcheckerResult result;

    public static Configuration impactedConfig;

    public static LogManager logManager;

    public static Configuration cpaConfig = null;
...
//        if(cpachecker.config.getProperty("cfa.export.line_number") != null) {
//            DOTBuilder.isLineNumber = Boolean.valueOf(cpachecker.config.getProperty("cfa.export.line_number"));
//        }
        if(cpachecker.config.getProperty("cpa.impacted.diff.cfaInfo") != null) {
            GlobalInfo.getInstance().cfaInfoPath = cpachecker.config.getProperty("cpa.impacted.diff.cfaInfo");
            ImpactedHelper.getInstance().cfaInfoPath = Paths.get(cpachecker.config.getProperty("cpa.impacted.diff.cfaInfo"));
        }
        ...
        // run analysis
        assert options.programs != null;
        program = options.programs;

        result = cpachecker.run(options.programs);
        ...
        try {
            if(MainCPAStatistics.programCpuTime > GlobalInfo.getInstance().cpuTimeLimit)
                result = new CPAcheckerResult(Result.UNKNOWN, "",
                    (ReachedSet) result.getReached(), result.getStats());
    ...
    private static Pair<Configuration, String> createConfiguration(String[] args) throws InvalidConfigurationException, InvalidCmdlineArgumentException, IOException {
        ...
        summaryOutputPath = cmdLineOptions.get("summaryOutputPath");
        if(cmdLineOptions.containsKey("loopSummary"))
            loopSummary = true;
        if(cmdLineOptions.containsKey("isHeuristicAddingMultiEdge"))
            isHeuristicAddingMultiEdge = true;
        if(cmdLineOptions.containsKey("onlyLoopSummary")) {
            onlyLoopSummary = true;
            loopSummary = true;
        }
        if(cmdLineOptions.containsKey("onlyFunctionSummary")) {
            onlyFunctionSummary = true;
            loopSummary = false;
        }
        if(!cmdLineOptions.containsKey("summaryInputPath")) {
            summaryInputPath = null;
        }
        else {
            summaryInputPath = cmdLineOptions.get("summaryInputPath");
        }
        if(cmdLineOptions.containsKey("isIncPred")) {
            isIncPred = true;
        }
        if(cmdLineOptions.containsKey("useTextSummary")) {
            isUesTextSummary = true;
        }
        if(cmdLineOptions.containsKey("LazyExpansion")) {
            noLazyExpansion = false;
        }
        if(cmdLineOptions.containsKey("noReduction")) {
            useReduction = false;
        }
        if(cmdLineOptions.containsKey("subgraph")) {
            isSubgraph = true;
        }
        if(cmdLineOptions.containsKey("analysis.entryFunction")) {
            entryFunction = cmdLineOptions.get("analysis.entryFunction");
        }
        ...
        ConfigurationBuilder impactedConfigBuilder = Configuration.builder();

        impactedConfigBuilder.loadFromFile("config/impactedAnalysis.properties");

        impactedConfigBuilder.setOptions(cmdLineOptions);
        impactedConfig = impactedConfigBuilder.build();
        ...
        Pair<Configuration, String> p1 = setupPaths(impactedConfig, secureMode);
        impactedConfig = p1.getFirst();
        // Check if we should switch to another config because we are analyzing memsafety properties.
        BootstrapOptions options1 = new BootstrapOptions();
        impactedConfig.inject(options1);
        if (options1.checkMemsafety) {
            if (options1.memsafetyConfig == null) {
                throw new InvalidConfigurationException("Verifying memory safety is not supported if option memorysafety.config is not specified.");
            }
            impactedConfig = Configuration.builder()
                .loadFromFile(options1.memsafetyConfig)
                .setOptions(cmdLineOptions)
                .clearOption("memorysafety.check")
                .clearOption("memorysafety.config")
                .clearOption("output.disable")
